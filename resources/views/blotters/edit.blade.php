...
@section('header_title', 'Edit Blotter Record: ' . $blotter->case_no)

@section('content')
<div class="card">
    <div class="card-header border-0 bg-transparent">
        <div class="d-flex align-items-center">
            <a href="{{ route('blotters.show', $blotter->blotter_id) }}" class="btn btn-sm btn-outline-secondary me-3">
                <i class="fas fa-arrow-left me-1"></i> Back
            </a>
            <h5 class="mb-0">Edit Blotter Record</h5>
        </div>
    </div>
    <div class="card-body">
        @if($errors->any())
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <select name="status" class="form-select" required>
                        @foreach(['Unresolved', 'Scheduled', 'Mediation', 'Settled', 'Dismissed', 'Forwarded to Court'] as $status)
                            <option value="{{ $status }}" {{ old('status', $blotter->status) == $status ? 'selected' : '' }}>
                                {{ $status }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <select name="complainant_id" class="form-select" required>
                        ...
                        @foreach($residents as $r)
                            <option value="{{ $r->resident_id }}" {{ old('complainant_id', $blotter->complainant_id)==$r->resident_id?'selected':'' }}>
                                {{ $r->full_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <select name="respondent_id" class="form-select" required>
                        ...
                        @foreach($residents as $r)
                            <option value="{{ $r->resident_id }}" {{ old('respondent_id', $blotter->respondent_id)==$r->resident_id?'selected':'' }}>
                                {{ $r->full_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <input type="date" name="incident_date" class="form-control" value="{{ old('incident_date', \Carbon\Carbon::parse($blotter->incident_date)->format('Y-m-d')) }}" required>
                </div>
                <div class="col-md-4">
                    <label class="form-label">Incident Location</label>
                    <input type="text" name="incident_location" class="form-control" value="{{ old('incident_location', $blotter->incident_location) }}" placeholder="Where the incident happened">
                </div>
                <div class="col-12">
                    <label class="form-label">Narrative / Details <span class="text-danger">*</span></label>
                    <textarea name="narrative" class="form-control" rows="6" required placeholder="Provide a detailed account of the incident...">{{ old('narrative', $blotter->narrative) }}</textarea>
                </div>
            </div>

            <div class="mt-4 pt-3 border-top d-flex justify-content-end gap-2">
                <a href="{{ route('blotters.show', $blotter->blotter_id) }}" class="btn btn-light border">Cancel</a>
